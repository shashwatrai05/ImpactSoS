...
function sendSOS() {
  ...
  function curPosition(position) {
    ...
    fetch("/api/report", {
      ...
      .then((data) => {
        if (data.success) {
          fetch(`/api/user?token=${localStorage.getItem("accessToken")}`)
            .then((res) => res.json())
            .then((data) => {
              const message = `${
                data.name
              } may have been involved in accident. They've listed this number as their emergency contact under the relation "${
                data.emergency_contact.relation
              }". Here's their live location: https://maps.google.com/?q=${
                (position.coords.latitude, position.coords.longitude)
              }. Powered by Mapcident`;
...
<i data-feather="radio" id="inside-icon"></i>
<a href="/dashboard">
